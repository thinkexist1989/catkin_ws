...
int main(int argc, char** argv)
{
  ...
  std::cout << "IP:" << argv[1] << "  PORT:" << portno <<std::endl;
  ...
  pRecvParam->sockfd = sockfd;
  pRecvParam->motor = motor;
  pRecvParam->sensor = sensor;
  pRecvParam->platform = platform;
  if(!pthread_create(&id,NULL,RecvThread,(void *)pRecvParam)){
   // printf("Create Thread Success!Thread id is %d",(int)id);
    std::cout << "Create Thread Success!Thread id is " << id <<std::endl;
  }
  else{
...
void motorctrlCallback(const tcp_test::motorctrl::ConstPtr& msg)
{
  ROS_INFO("%d=>motor control, motor1 pwm is: %f",cout++,msg->pwm[0]);
...
void *RecvThread(void *arg)
{
  int sockfd = ((RECVPARAM*)arg)->sockfd;
  ros::Publisher motor = ((RECVPARAM*)arg)->motor;
  ros::Publisher sensor = ((RECVPARAM*)arg)->sensor;
  ros::Publisher platform = ((RECVPARAM*)arg)->platform;
